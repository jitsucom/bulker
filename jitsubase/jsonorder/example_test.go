...
func ExampleConfigDefault_Marshal() {
	type ColorGroup struct {
		ID     int
		Name   string
		Colors []string
	...
	stream := ConfigDefault.BorrowStream(nil)
	defer ConfigDefault.ReturnStream(stream)
...
func ExampleConfigDefault_Unmarshal() {
	var jsonBlob = []byte(`[
		{"Name": "Platypus", "Order": "Monotremata"},
		{"Name": "Quoll",    "Order": "Dasyuromorphia"}
	]`)
	...
	var animals []Animal
	iter := ConfigDefault.BorrowIterator(jsonBlob)
	defer ConfigDefault.ReturnIterator(iter)
...
func ExampleGet() {
	...
	fmt.Print(Get(val, "Colors", 0).ToString())
	// Output:
	// Crimson
}
